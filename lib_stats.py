...
def lregress(x, y, tcrit=0.0):
    ...
    rc = cov_xy / cov_xx # regression coefficient (rc)
    se = ( cov_yy - (rc**2) * cov_xx ) * (nsamp-1) / (nsamp-2) # total standard error squared (se)
    sb = _np.sqrt( se / (cov_xx * (nsamp-1)) ) # standard error on rc (sb)
    eb = tcrit * sb # error bar on rc

    if ( (_np.abs(rc) - _np.abs(eb)) > 0.0 ): ssig = True
    else:                                     ssig = False
