...
class Projection(object):
    ...
    def __init__(self, projection='miller', \
        ...
        if   self.projection == 'stere':
            ...
            self.lon_0            = 0.0       if ( cenlon           == None ) else cenlon
            self.meridians_labels = [0,0,1,1] if ( meridians_labels == None ) else meridians_labels
            if (self.projection == 'npstere'):
                self.boundinglat      = 45.0      if ( boundinglat      == None ) else abs(boundinglat)
                self.parallels_labels = [1,0,0,0] if ( parallels_labels == None ) else parallels_labels
            if (self.projection == 'spstere'):
                self.boundinglat      = -45.0     if ( boundinglat      == None ) else -abs(boundinglat)
                self.parallels_labels = [0,1,0,0] if ( parallels_labels == None ) else parallels_labels

        elif ( self.projection in ['mill','miller','merc','mercator'] ):
            self.llcrnrlon        =    0.0 if ( llcrnrlon == None ) else llcrnrlon
            self.llcrnrlat        =  -90.0 if ( llcrnrlat == None ) else llcrnrlat
            self.urcrnrlon        =  360.0 if ( urcrnrlon == None ) else urcrnrlon
            self.urcrnrlat        =   90.0 if ( urcrnrlat == None ) else urcrnrlat
            self.meridians        = range(-180, 180, mlabel_int)
            self.parallels        = range(-90 ,90,   plabel_int)
            self.meridians_labels = [0,0,0,1] if ( meridians_labels == None ) else meridians_labels
            self.parallels_labels = [1,0,0,0] if ( parallels_labels == None ) else parallels_labels

        elif ( self.projection in ['lcc', 'lambert'] ):
            self.cenlat    = 0.0  if ( cenlat  == None ) else cenlat
            self.cenlon    = 0.0  if ( cenlon  == None ) else cenlon
            self.stdlat1   = 0.0  if ( stdlat1 == None ) else stdlat1
            self.stdlat2   = 0.0  if ( stdlat2 == None ) else stdlat2
            self.width     = 8.0  if ( width   == None ) else width
            self.height    = 11.0 if ( height  == None ) else height
            self.meridians        = range(-180, 180, mlabel_int)
            self.parallels        = range(-90 ,90,   plabel_int)
            self.meridians_labels = [0,0,0,1] if ( meridians_labels == None ) else meridians_labels
            self.parallels_labels = [0,1,0,0] if ( parallels_labels == None ) else parallels_labels
...
def createMap(proj, **kwargs):
    ...
    if proj.projection == 'stere':
        map = _Basemap(projection = proj.projection, **kwargs)

    elif ( proj.projection in ['npstere','spstere'] ):
        map = _Basemap(projection  = proj.projection, \
...
def drawMap(map,proj,coastlines=True,fillcontinents=True,meridians=True,parallels=True,**kwargs):
    '''
    Draw map with coastlines, fill continents, draw meridians and parallels, etc.
    '''

    if ( coastlines     ): map.drawcoastlines(color='dimgray')
    if ( fillcontinents ): map.fillcontinents(color='lightgray',lake_color='white')
    if ( meridians      ): map.drawmeridians(proj.meridians,labels=proj.meridians_labels)
    if ( parallels      ): map.drawparallels(proj.parallels,labels=proj.parallels_labels)
