...
def wrf_proj(filename):
    ...
    if ( 'bottom_top' in nc.dimensions ): proj.nz = len(nc.dimensions['bottom_top'])
    proj.dx        = float(nc.DX)
    proj.dy        = float(nc.DY)
...
def proj_set(proj):
    ...
    if ( proj.stdlat1 < 0.0 ): proj.hemi = -1
    else:                      proj.hemi = 1
    ...
    if   ( proj.code == 0 ): # latitude longitude
        ...
        if ( dlon1 >  180.0 ): dlon1 = dlon - 360.0
        if ( dlon1 < -180.0 ): dlon1 = dlon + 360.0
        ...
        ala1 = proj.lat1 * pid
        proj.rsw = proj.rebydx * _math.cos(ala1) * s_top / (1.0 + proj.hemi * _math.sin(ala1))
        ...
        proj.rsw = 0.0
        if ( proj.lat1 != 0.0 ):
            proj.rsw = (_math.log(_math.tan(0.5*((proj.lat1+90.0)*pid)))) / proj.dlon
...
def latlon_to_ij(lat,lon,proj):
    ...
    if   ( proj.code == 0 ): # latitude-longitude grid
        ...
        s_top = 1.0 + proj.hemi * _math.sin(proj.stdlat1 * pid)
        ala = lat * pid
        rm = proj.rebydx * _math.cos(ala) * s_top / (1.0 + proj.hemi * _math.sin(ala))
        alo = (lon - reflon) * pid
        ...
        jr = 1.0 + (_math.log(_math.tan(0.5*((lat + 90.0) * pid)))) / proj.dlon - proj.rsw

    else:
        raise Exception('unknown proj.code for proj_set')

    ir = int(_np.round(ir)) - 1 # acknowledge the fact that in python indexing starts at 0
    jr = int(_np.round(jr)) - 1 # acknowledge the fact that in python indexing starts at 0
...
def ij_to_latlon(i,j,proj):
    ...
    if   ( proj.code == 0 ): # latitude-longitude grid
        ...
        if ( r2 == 0.0 ):
            ...
            lon = proj.stdlon + _math.atan2(proj.hemi*xx,yy) / proj.cone / pid
            lon = _np.mod(lon + 360.0,360.0)

            if ( chi1 == chi2 ):
                chi = 2.0 * _math.atan( ((r          /_math.tan(chi1)) ** (1.0 / proj.cone)) * _math.tan(chi1 * 0.5) )
            else:
                chi = 2.0 * _math.atan( ((r*proj.cone/_math.sin(chi1)) ** (1.0 / proj.cone)) * _math.tan(chi1 * 0.5) )
        ...
        lat = 2.0 * _math.atan(_math.exp(proj.dlon * (proj.rsw + j - 1.0))) / pid - 90.0
...
def find_min_value(fldin, glat, glon, proj, radius=10):
    ...
    xrange = _np.arange(_np.max((ir-radius,0)), _np.min((ir+radius,proj.nx)), 1)
    yrange = _np.arange(_np.max((jr-radius,0)), _np.min((jr+radius,proj.ny)), 1)
...
def find_min_max(fldin, glat, glon, proj, radius=10, minima=True):
    ...
    if ( minima ):
        [iout, jout, val] = find_min_value(fldin, glat, glon, proj, radius=radius)
    else:
        [iout, jout, val] = find_max_value(fldin, glat, glon, proj, radius=radius)
